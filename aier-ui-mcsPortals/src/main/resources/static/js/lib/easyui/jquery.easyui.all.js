...
//				}
			...
			if (td.length){
				...
				if(!tt.hasClass('s-op') || tt.attr('opt')=='button'){opts.onClickCell.call(target, index, field, row[field]);}
			}
	...
	function endEdit(target, index, cancel){
		...
		if (!cancel){
			...
			tr.find('div.datagrid-editable').each(function(){
				...
				var value = ed.actions.getValue(ed.target).replace(/<script/g,"＜script").replace(/script>/g,"script＞");
				// console.log(value);
				if (row[field] !== value){
					row[field] = value;
					changed = true;
...
//				clear(this);
	...
	$.fn.combo.defaults = $.extend({}, $.fn.textbox.defaults, {
		...
		hasDownArrow: true,
		minRemoteKeyLen:2,
		delay: 400,	// delay to do searching from the last key input event.
