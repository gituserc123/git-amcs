(function () {
  ...
  IMsg.prototype = {
    ...
    initData : function (callback){//消息 初始化显示
      ...
      $.when($.ajax({
        url : base+'/ui/sys/sysMessage/getMessagePush',type: "post"
      }), $.ajax({
        url : base+'/ui/sys/sysMessage/getInpOrderMsgPush',type: "post"
      })).done(function(rst1,  rst2){
          // var rst = temData;
          var mainData = rst1[0].data;
          var ahisData = rst2[0].data;
        // console.log(mainData,ahisData);
          var allData = [];
          $.each(mainData,function (i,v){
            if(v.bizType!=='AHIS_1'){allData.push(v);}
          });
        allData = allData.concat(ahisData);
        // console.log(allData);
          // $d.msgBox.show();//显示消息
          ...
          $.each(allData,function (i,v){//处理消息，合并到 msgs 消息集中
          ...
          me.setBadgeState(allData.length - reduceTotalNum);//更新badge
    ...
    actionType : {//分类 处理函数
      ...
      AHIS_1 : function (msgBox,msg,state){ // AHIS_1
        ...
        if(state){//新增
            if(totalS>0 && (cont.isDoctor==1?true:cont.currentWardName == userNowDept)){//状态 总和大于0 新增 ，等于0忽略 ,并且 是（医生 或者 不是医生但是消息是对应当前科室）
              msgBox.size++;
    ...
    receiveMsg : function (evMsg) {//接收消息处理
      ...
      if(evMsg.msg.content) {
        ...
        me.receiveMsgType = cont.bizType;
        console.log('msgCont',cont);
        me.msgStateType = cont.state;
        if(cont.bizType=='ALIS_CV'){
          var cvKeys = Object.keys(msgs.ALIS_CV.data);
          console.log(cvKeys,'cont.bizId:',cont.bizId);
          if(cvKeys.indexOf(cont.bizId)>-1){me.msgStateType = '2'};//如果已存在列表中，标记为已读
        }
        me.getDom(); //初始化dom对象
    ...
    popCvInfo: function (){
      var me = this;
      if (me.cTimer){clearTimeout(me.cTimer);}
      me.cTimer = setTimeout(function (){
        me.runPopCvInfo();
      },800);
    },
    runPopCvInfo: function (){
      var me = this;
      var alisCvKeys = Object.keys(msgs.ALIS_CV.data);
      me.clearCVTimer();
      if(me.cvPop){layer.close(me.cvPop);me.cvPop = null;}//如果已经有了弹窗，则先关闭弹窗，再弹新的窗口
      if($('.cvMsgPop').length){
        $('.cvMsgPop').each(function (){
          layer.close($(this).attr('times'));
        });
      }
      // console.log(Object.keys(msgs.ALIS_CV.data).join(','));
      if(alisCvKeys.length){//如果消息条数大于0
        me.cvPop = layer.open({
          type: 2,
          skin:'cvMsgPop',
          content : 'alis/ui/alis/report/criticalValue/handle?bizIds='+alisCvKeys.join(','),
          title : '危急值',
          area : ['800px','460px'],
          end: function (){
            me.cvPop = null;
            alisCvRole && me.runCvTimer();
          }
        });
        console.log('cvPop',me.cvPop);
      }
    },
    cvPop:null,
    cvTimer:null,
    clearCVTimer: function (){//清除危急值倒计时
      var me = this;
      if(me.cvTimer){
        window.clearInterval(me.cvTimer);
        me.cvTimer = null;
      }
    },
    runCvTimer: function (){//开启危急值倒计时
      var me = this;
      var endS = 1200;
      me.clearCVTimer();
      me.cvTimer = window.setInterval(function (){
        endS--;
        if(endS<=0){
          me.popCvInfo();
        }
    ...
    alwaysFlashMsg : function (bizType){// (ALIS_CV 危急值) || ( AHIS_1中数据(urgent>0)) 时总闪动
      ...
      // var alisMsgKeys = Object.keys(msgs.ALIS_CV.data);
      for (k in msgs.ALIS_CV.data){//有危急值就要总闪动
        if(k){needFlash = 1; hasWei = 1;}//如果有值
      }
      ...
      $d.msgBox[hasWei?'addClass':'removeClass']('hasWeiMark');
      //检验科 用户，有危急值直接弹窗显示
      // console.log('alisCvRole:',alisCvRole,',hasWei:',hasWei,',receiveMsgType:',me.receiveMsgType);
      if( hasWei && ((me.receiveMsgType=='ALIS_CV' && me.msgStateType!=='2') || me.receiveMsgType=='init')){// 有危急值 && ((这次推过来的是危急值 && 且不为已读状态) || 初始状态)
        me.msgStateType = '2';
        me.popCvInfo();
      }
      me.clearCVTimer();
      me.receiveMsgType = null;
