...
<div class="searchHead">
	<form id="sbox" class="soform form-enter">
		...
		<select class="drop easyui-combobox txt-validate required"  style="width: 100px" type="text" name="paraType" id="paraType" value="" noNull="必填">
			[#list dictTypes as item]
			<option value="${item.typecode}" selected>${item.typedesc}</option>
			[/#list]
...
<script type="text/javascript">
	requirejs(["pub"], function () {
		...
		$grid.newGrid("#gridBox", {
			...
			queryParams: {"paraType":$("#paraType").getVal()},
			checkOnSelect: false,
			selectOnCheck: false,
			singleSelect: false,
			pagination:false,
