	...
	<form class="soform formA form-validate">
        ...
        <div class="clearfix mar-l20 mar-b10">
            <span class="fl">
                ...
                <input id="admissionDate" class="txt txt-validate so-date" autocomplete="off" type="text" style="width:100px" name="admissionDate" dataOptions="editable:false,required:ture, maxDate:new Date(),format:'yyyy-MM-dd',type:'date'" noNull="请填写入院时间" value="[#if ae.admissionDate??]${ae.admissionDate?date("yyyy-MM-dd")}[/#if]"  />
            </span>
            <span class="fl">
                <label class="lab-inline">出院时间：</label>
                <input id="dischargeDate" class="txt txt-validate so-date" autocomplete="off" type="text" style="width:100px" name="dischargeDate" value="[#if ae.dischargeDate??]${ae.dischargeDate?date("yyyy-MM-dd")}[/#if]"  />
            </span>
			<span class="fl">
                <label class="lab-inline">治疗时间：</label>
                <input id="treatDate" class="txt txt-validate so-date" autocomplete="off" type="text" style="width:100px" name="treatDate" dataOptions="editable:false,required:false, maxDate:new Date(),format:'yyyy-MM-dd',type:'date'"  value="[#if ae.treatDate??]${ae.treatDate?date("yyyy-MM-dd")}[/#if]"  />
            </span>
            <span class="fl">
	...
	<script type="text/javascript">
        requirejs(["eventReview", "eventTableGroup","eventFlowchart","uploadImages","eventImage",'myupload','pub'], function (eventReview, eventTableGroup,eventFlowchart,uploadImages,eventImage) {
	        ...
	        window.afterSerachPatient=function (){
				...
				if(opDomain){
					var opEmrUrl = !!opDomain?opDomain.emrUrl:"";
					if(opEmrUrl){
						$('.btn-aemr').removeClass('hide');
						$('.btn-aemr').click(function () {
							window.open(opEmrUrl,'_blank');
						});
					}else{
						$('.btn-aemr').addClass('hide');
					}
					$('#outpDiagName').setVal(!!opDomain?opDomain.diagName:"")
				}
				//住院接口数据
	            var inDomain=window.inDomain;
				if(inDomain){
					$('#admissionDate').setVal(!!inDomain.inpTime?inDomain.inpTime.substr(0,10):"")
					// 出院时间
					if(inDomain.dischargeTime){
						$('#dischargeDate').setVal(!!inDomain.dischargeTime?inDomain.dischargeTime.substr(0,10):"")
						// 出院视力类型
					...
					$('#inpDiagName').setVal(!!inDomain?inDomain.diagName:"")
					...
					$('#admissionIopOd1').setVal(!!inDomain.aemrVisionMap.admission.rightVision?inDomain.aemrVisionMap.admission.rightVision:"")
					$('#admissionIopOs1').setVal(!!inDomain.aemrVisionMap.admission.leftVision?inDomain.aemrVisionMap.admission.leftVision:"")
					...
					var emrUrl =!!inDomain?inDomain.emrUrl:"";
					if(emrUrl){
						$('.btn-aemr').removeClass('hide');
						$('.btn-aemr').click(function () {
