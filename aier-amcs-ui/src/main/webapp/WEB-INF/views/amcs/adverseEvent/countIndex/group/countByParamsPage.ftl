...
requirejs(['pub'],function () {
    let queryParam = '${cond}';
    // 判断 queryParam 是否是有效的 JSON 字符串
    let queryObject = {};
    if (queryParam) {
        try {
            // 将 queryParam 转换为 JavaScript 对象
            queryObject = JSON.parse(queryParam);
        } catch (e) {
 ...
 $grid.newGrid("#gridBox",{
  tools:[
          ...
          url:'${base}/ui/amcs/countIndex/countByParamsPageGrid',
          ...
          columns:[[
            ...
            {title:'省区',field:'province',width:120},
            {title:'医院',field:'hospName',width:200},
			...
			{title:'赔偿金额(元)',field:'compensationamount',width:100},
            ...
            [#if groupType== 'groupProvReviewOver10Days']
            {title:'超期天数',field:'daysdiff',width:80}
            [/#if]
          ]],
