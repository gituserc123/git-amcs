...
<div id="hello">
  ...
  <div class="p-10 m-10 bg-white">
    ...
    <div id="echart-provinceByGradeOne" style="height: 300px;width: 100%;"></div>
  </div>
  
  <div class="p-10 m-10 bg-white">
    <div style="color: #262626;font-size: 14px;border-bottom: 1px solid #ccc;font-weight: bold;" class="pad-l15 pad-b10">医院不良事件上报分布图</div>
    <div id="echart-provinceByHospId" style="height: 300px;width: 100%;"></div>
  </div>
...
<script type="text/javascript">
    ...
    var label_info = [
        ...
        {img:"icon-bianjiwenjian",label:'待省区审批',count:'',color:'#e98f36',url:{title:"省区不良事件管理",uri:"${base}/ui/service/biz/amcs/adverse/common/listPage?page_type=3"}},
        {img:"icon-zhanghutuikuan",label:'被回退',count:'',color:'#7cba59',url:{title:"省区不良事件跟踪",uri:"${base}/ui/service/biz/amcs/adverse/common/listTrackPage?page_type=3&is_index=1"}},
        {img:"icon-biaodanwancheng2",label:'归档/完成',count:'',color:'#17abe3'},
        {img:"icon-weishangbao",label:'未完结',count:'',color:'#db639b',url:{title:"未完结列表",uri:"${base}/ui/amcs/countIndex/unfinishedProvince"}},
        {img:"icon-shenhetongguo",label:'未完结(超过90天)',count:'',color:'#ed7d31',url:{title:"未完结(超过90天)列表",uri:"${base}/ui/amcs/countIndex/unfinished90Province"}},
        {img:"icon-genzong",label:'省区审核通过',count:'',color:'#7cba59'},
        {img:"icon-fengxiandengji-",label:'高风险',count:'',color:'#db639b'},
        {img:"icon-zelvxingzhengjiufen",label:'纠纷',count:'',color:'#17abe3',url:{title:"纠纷列表",uri:"${base}/ui/amcs/countIndex/disputeProvince"}},
        {img:"icon-dingdan",label:'完结',count:'',color:'#e98f36'},
    	//{img:"icon-shenhebutongguo",label:'未点评',count:'',color:'#7cba59'},
        {img:"icon-bg-money",label:'赔偿',count:'',color:'#17abe3',url:{title:"赔偿列表",uri:"${base}/ui/amcs/countIndex/compensationProvince"}},
        {img:"icon-shenpi",label:'超时上报',count:'',color:'#db639b'},
        {img:"icon-shenhebutongguo",label:'事件进展',count:'',color:'#7cba11',url:{title:"事件进展列表",uri:"${base}/ui/amcs/countIndex/eventsProgressProvince"}},
      ];
    ...
    requirejs(["echarts",'vue', 'pub'], function (echarts,Vue) {
        ...
        let App = new Vue({
            ...
            mounted() {
                ...
                this.initProvinceByGradeOne();
                this.queryProvinceByGradeOne();
				
                this.initProvinceByHospId();
                this.queryProvinceByHospId();
            ...
            methods:{
                initResize(){
                    ...
                    window.onresize = function(){
                        $this.echart_provinceByGradeOne && $this.echart_provinceByGradeOne.resize();
                        $this.echart_provinceByHospId && $this.echart_provinceByHospId.resize();
                        $this.echart_sec && $this.echart_sec.resize();
                        $this.echart_event && $this.echart_event.resize();
                    }
                },
                initProvinceByGradeOne(){
                    var dom_provinceByGradeOne = document.getElementById("echart-provinceByGradeOne");//不良事件类型分布图 
                    var echart_provinceByGradeOne = echarts.init(dom_provinceByGradeOne);
                    echart_provinceByGradeOne.setOption(eOpt);
                    this.echart_provinceByGradeOne = echart_provinceByGradeOne;
                },
				initProvinceByHospId(){
                    var dom_provinceByHospId = document.getElementById("echart-provinceByHospId");//医院不良事件上报分布图
                    var echart_provinceByHospId = echarts.init(dom_provinceByHospId);
                    echart_provinceByHospId.setOption(eOpt);
                    this.echart_provinceByHospId = echart_provinceByHospId;
                },
                initSec(){//亚专科
                ...
                query(){
                    ...
                    this.queryProvinceByGradeOne(params);
                     this.queryProvinceByHospId(params);
                ...
                queryTopIncident(params){
                    $ajax.post({
                        url:"${base}/ui/amcs/countIndex/province",//省区
                        data:params,
                ...
                queryProvinceByGradeOne(params){
                    $ajax.post({
                        url:"${base}/ui/amcs/countIndex/provinceByGradeOne",
                        data:params,
                        jsonData : false,//是否采用jsonData格式提交
                        type : 'post',//采用post方式提交
                        callback :(rst)=>{
                            if(rst && rst.length>0){
                                ...
                                this.echart_provinceByGradeOne.setOption(eOpt);
                            }else{
                            	var eOpt = {
                                    xAxis: {
                                        data: [],
				...
				queryProvinceByHospId(params){
                    $ajax.post({
                        url:"${base}/ui/amcs/countIndex/provinceByHospId",//医院上报
                        data:params,
                        jsonData : false,//是否采用jsonData格式提交
                        sync : false,//是否同步方式提交
                        type : 'post',//采用post方式提交
                        callback :(rst)=>{
                            if(rst && rst.length>0){
                                ...
                                this.echart_provinceByHospId.setOption(eOpt)
                            }else{
                            	var eOpt = {
                                    xAxis: {
                                        data: [],
                ...
                querySec(params){
                    $ajax.post({
                        url:"${base}/ui/amcs/countIndex/provinceBySubspecialty",
                        data:params,
                        jsonData : false,//是否采用jsonData格式提交
                        sync : false,//是否同步方式提交
                        type : 'post',//采用post方式提交
                ...
                queryEvent(params){
                    $ajax.post({
                        url:"${base}/ui/amcs/countIndex/provinceByEventLevel",
                        data:params,
                        jsonData : false,//是否采用jsonData格式提交
                        sync : false,//是否同步方式提交
                        type : 'post',//采用post方式提交
