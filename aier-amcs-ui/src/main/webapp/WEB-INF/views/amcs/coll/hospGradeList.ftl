...
<script id="editTem" type="text/html">
     <form class="soform form-validate formColl pad-t20">
        ...
        <input type="hidden" id="gradeLevelName" name="gradeLevelName" value="${gradeLevelName!}" />
        ...
        <div class="row">
            ...
            <div class="p3">
                <div class="item-one">
                    ...
                    <select class="drop easyui-combobox" style="width:100%;" name="grade" id="grade" data-options="editable:false, required:true,value:'${grade}'" >
                    	<option value=""></option>
                        <option value="0">未定级</option>
            ...
            <div class="p3 gradeLevelDiv">
                <div class="item-one">
                    <label class="lab-item">已评等次</label>
                    [@ui_select id="gradeLevel" name="gradeLevel" tag="hosp_level" style="width:100%;" dataOptions="editable:false,required:true" filterkey="firstSpell" value="${hospLevel }"/]
                </div>
            </div>
        </div>
        <div class="row">
            ...
            <div class="p6">
                <div class="item-one">
                    ...
                    [@ui_select id="applyGrade" name="applyGrade" tag="hosp_grade" style="width:100%;" dataOptions="editable:false,required:true" filterkey="firstSpell" value="${applyGrade }"/]
                </div>
            </div>
        </div>
        <div class="row">
...
<script type="text/javascript">
	requirejs(['lodash', "easygridEdit", "uploadImages", 'myupload', 'pub'], function (_, $e, uploadImages) {
        ...
        $grid.newGrid("#gridBox",{
            ...
            tools: [[
                {
                    iconCls: 'plus', text: '新增',click:function (){ popTem('医院等级填报', null, 'add', null);}
			    }
			]],
	    	columns:[[
                ...
                {title:'已评等次', field:'gradeLevelName', width:200},
		    ...
		    onLoadSuccess : function (data) {
		    	$('.s-op-view').click(function () {
					...
					popTem('医院等级查看', id, 'view', ix)
				});
				$('.s-op-modify').click(function () {
					var ix = $(this).attr('rel');
					var id = data.rows[ix].id;
					popTem('医院等级编辑', id, 'edit', ix)
				});
            },
            queryParams : {},
      		url: '${base}/ui/amcs/coll/hosp/grade/findList'
  		});
        ...
        function popTem(title, id, op, index){
            ...
            var popTem = $pop.popTemForm({
                ...
                onPop:function(){
                    ...
                    if(op == 'view' || op == 'edit') {
                    	...
                    	$.ajax({
                            url : "${base}/ui/amcs/coll/hosp/grade/findById",
                            data : { id : id },
                            type: "post"
                        }).done(function (res) {
                        	res.grade+="";
            ...
            $('.btn-save').click(function () {
                ...
                if(validate){
	            	...
	            	$ajax.post('${base}/ui/amcs/coll/hosp/grade/save', formData,{jsonData:true,tip:false}).done(function(rst){
            ...
            $('#grade').combobox({
				onChange: function(newValue,oldValue){
                    ...
                    if (newValue == '0') {
                        $('.gradeLevelDiv').css('visibility', 'hidden');
                        $('#gradeLevel').combobox('disableValidation');
                    } else {
                        $('.gradeLevelDiv').css('visibility', 'visible');
                        $('#gradeLevel').combobox('enableValidation');
                    }
                    //gradeLevelCombo.combobox('validate');
				}
			});
			...
			$('#gradeLevel').combobox({
				onChange: function(newValue,oldValue){
					$('#gradeLevelName').val($('#gradeLevel').combobox('getText'));
				}
			});
