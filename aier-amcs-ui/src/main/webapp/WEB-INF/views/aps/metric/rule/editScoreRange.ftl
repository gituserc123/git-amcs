...
<div class="wrapper">
    ...
    <p class="row-btn center bot-line-sub">
        <input type="button" class="btn btn-primary btn-edit" name="btnSubmit" value="确   定" />
    ...
    require(["easygridEdit","pub"],function($e) {
        ...
        $grid.newGrid("#gridBox",{
            ...
            columns:[[
                ...
                {title:'最小值',field:'minRange',width:'80px',editor:{type:'diy',options:{validType:'rangeNum[0, 999999, 6,false, false, ""]',required:true,missingMessage:'请填写'}}},
                {title:'最大值(含)',field:'maxRange',width:'80px',editor:{type:'diy',options:{validType:'rangeNum[0, 999999, 6,false, false, ""]',required:true,missingMessage:'请填写'}}},
                {title:'分值/比例',field:'rangeScore',width:'120px',editor:{type:'diy',options:{validType:'rangeNum[0, 999999, 6,false, false, ""]',required:true,missingMessage:'请填写标准分值/比例'}}},
        ...
        $('.btn-edit').click(function () {
            if($('#sbox').form('validate')&&$e.validateGrid()){
                ...
                for(var i=0;i<rowsData.length;i++){
                	...
                	if(lastv > parseFloat(rowsData[i].minRange)){
                		$pop.alert('第'+(i+1)+'行最小值有误，请从小到大的顺序维护，且范围不得有交叉！');
	            		return false;
                	}else{
                		lastv = parseFloat(rowsData[i].minRange)
                	}
                	
                	if(lastv > parseFloat(rowsData[i].maxRange)){
                		$pop.alert('第'+(i+1)+'行最大值有误，请从小到大的顺序维护，且范围不得有交叉！');
	            		return false;
                	}else{
                		lastv = parseFloat(rowsData[i].maxRange)
                	}
	            }
