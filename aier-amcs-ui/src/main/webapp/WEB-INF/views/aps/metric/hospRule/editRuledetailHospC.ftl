    ...
    <script id="batchSet" type="text/html">
	    <form class="soform formA form-validate form-enter form-setFangStop pad-t25" action="" >
	        <div class="row">
	        	<div class="p6">
	                <div class="item-one">
	                 <label class="lab-item">麻醉方式：</label>
					[@ui_select uiShowDefault=false class="drop easyui-combobox" uiShowDefault=true uiShowDefaultDesc="全部" value="" id="oprLevel" name="oprLevel" style="width:100%;" tag="anesthesia_mode" filterkey="" dataOptions="
					limitToList:true,reversed:true,panelHeight:'auto',panelMaxHeight:200,
					onSelect : function(v){
						if(v.valueCode == ''){
							return 
						}
						if(v.valueCode == '1' || v.valueCode == '3' || v.valueCode == '13' || v.valueCode == '35' || v.valueCode == '36'){
							$('#oprIcd').combobox('loadData',  icdData); 
						}else{
							$('#oprIcd').combobox('loadData', [{valueCode:'',valueDesc:'全部'}]); 
						}
					},
	            ...
	            <div class="p6">
					...
					[@ui_select uiShowDefault=false class="drop easyui-combobox" uiShowDefault=true uiShowDefaultDesc="全部"  value="" id="oprIcd" name="oprIcd" style="width:100%;" tag="performance_diseases" filterkey="" dataOptions="limitToList:true,reversed:true,panelHeight:'auto',panelMaxHeight:200,
					onLoadSuccess : function(){
					if(typeof icdData === 'undefined'){
						icdData = $('#oprIcd').combobox('getData');
					}
					},"/]	                
	                </div>
	            </div>
    ...
    require(["easygridEdit","pub"],function($e) {
        ...
        $grid.newGrid("#gridBox",{
                      ...
                      var popId = $pop.popTemForm({
						...
						beforeSubmit : function (opt,$form, data) {
		                    ...
		                    $.each(chkRows,function(i,v){
		                    	var itemLevel = v.itemCode.split("_")[0]
		                    	var itemIcd = v.itemCode.split("_")[1]
		                    	if(data.oprLevel == ''){
		                    		if(data.oprIcd == ''){
		                    			v.itemScore = data.itemScore
		                    		}else if(data.oprIcd == itemIcd){
		                    			v.itemScore = data.itemScore
		                    		}
		                    	}else if(itemLevel == data.oprLevel){
		                    		if(data.oprIcd == ''){
		                    			v.itemScore = data.itemScore
		                    		}else if(data.oprIcd == itemIcd){
		                    			v.itemScore = data.itemScore
            ...
            columns:[[
                ...
                {title:'麻醉-病种编码',field:'groupItemCode',width:'90',editor:{type:'readonly'}},
                {title:'麻醉-病种名称',field:'groupItemName',align:'left',width:'181px',editor:{type:'readonly'}},
                ...
                {title:'医院标准分值/比例',field:'itemScore',width:'120px',editor:{type:'diy',options:{validType:'rangeNum[0, 99999, 6,false, false, ""]',required:false,missingMessage:'请填写'}}},
                ...
                /**
                {title:'是否启用',field:'usingSign',width:65,editor:{
                        type: 'combobox', options: {
                            ...
                            panelHeight:"auto",
                            panelMaxHeight:200,
                            data : [{valueCode:1, valueDesc: "是"},{valueCode:0, valueDesc: "否"},],
                            required: true,
							 missingMessage:'请选择',
                            onSelect : function (record){
                            },
