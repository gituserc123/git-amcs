...
<body>
<div class="pad-20">
  <form class="soform form-validate form-enter solab-l mar-b10" method="post" action="${base}/ui/aps/meritsConfigs/update">
  	<input type="hidden" name="id" value="${meritsConfigs.id}"/>
    <div class="row">
    ...
    <div class="row">
      <div class="p12"><div class="item-one">
          ...
          [@ui_select id="usingSign" name="usingSign" tag="com.aier.cloud.basic.api.domain.enums.UsingSignEnum"
           style="width:100%;" dataOptions="editable:false,limitToList:true" uiShowDefault="false" value="${meritsConfigs.usingSign}"][/@ui_select]
      </div></div>
    </div>
    <p class="row-btn">
...
require(["editor", "pub"], function (editor) {
        ...
        $('#meritsLevelId').combogrid('setValue',{ID:'${(meritsConfigs.meritsLevelId)!}',OPERATIONCLASSIFY:'${(meritsConfigs.meritsLevelName)!}'});
        $('#mainOperation').combogrid('setValue',{ID:'${(meritsConfigs.mainOperation)!}',OPERATIONCLASSIFY:'${(meritsConfigs.mainOperationName)!}'});
        $('#lessOperation').combogrid('setValue',{ID:'${(meritsConfigs.lessOperation)!}',OPERATIONCLASSIFY:'${(meritsConfigs.lessOperationName)!}'});
