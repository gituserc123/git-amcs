...
import com.aier.cloud.api.amcs.fin.domain.FinMaDrgDtl;
...
import com.aier.cloud.biz.ui.biz.fin.feign.FinMaDrgDtlFeignService;
...
@Controller
@RequestMapping("/ui/amcs/fin/drg/ma")
public class FinDrgManageAnalysisUiController extends BaseController {

    private static final String DRG_MA_PAGE = "amcs/fin/business/manageAnalysis/finDrgMaPage";
    private static final String DRG_STAT_MA_PAGE = "amcs/fin/business/manageAnalysis/finDrgStatMaPage";

    @Autowired
    private FinMaMainFeignService finMaMainFeignService;

    @Autowired
    private FinMaDrgDtlFeignService finMaDrgDtlFeignService;
    ...
    @RequestMapping(value = "/maDrgForm", method = { RequestMethod.GET, RequestMethod.POST })
    public String drgMaPage(Map<String, Object> map) {
        ...
        if(CollectionUtils.isNotEmpty(finMaMains) && finMaMains.size()>0) {
            ...
            FinMaDrgDtlCondition finMaDrgDtlCondition = new FinMaDrgDtlCondition();
            finMaDrgDtlCondition.setHospId(shiroUser.getTenantId());
            finMaDrgDtlCondition.setMaIdentifier(curQuarter+shiroUser.getTenantId());
            if(CollectionUtils.isNotEmpty(maMainIdList) && maMainIdList.size()>0) {
                finMaDrgDtlCondition.setMainIdList(maMainIdList);
            }
            List<FinMaDrgDtl> finMaDrgDtls = finMaDrgDtlFeignService.getAllRecord(finMaDrgDtlCondition);
            if(CollectionUtils.isNotEmpty(finMaDrgDtls) && finMaDrgDtls.size()>0) {
                finMaDrgDtls.stream().forEach(fmdp -> map.put(fmdp.getBusiSign(),fmdp));
            }
        }else{
            // 初次加载，自动反显医院信息，医保信息等
    ...
    @RequestMapping(value = "/saveDrg", method = { RequestMethod.GET, RequestMethod.POST })
    @ResponseBody
    public  Map<String, Object> saveDrg(@RequestBody String jsonData){
        return finMaMainFeignService.saveDrgStr(jsonData);
    }

    @RequestMapping(value = "/drgStatMaPage", method = { RequestMethod.GET, RequestMethod.POST })
    public String drgStatMaPage(Map<String, Object> map) {
        return DRG_STAT_MA_PAGE;
    }

    @RequestMapping(value = "/getDrgAll", method = { RequestMethod.POST })
    @ResponseBody
    public PageResponse<Map<String, Object>> getDrgAll(FinMaMainCondition cond){
        ...
        List<FinMaDrgDtl> finMaDrgDtls = null;
        Map<Long, List<FinMaDrgDtl>> groupedDetails;
        if(Objects.nonNull(idLists) && idLists.size()>0){
            FinMaDrgDtlCondition condDrg = new FinMaDrgDtlCondition();
            condDrg.setMainIdList(idLists);
            finMaDrgDtls = finMaDrgDtlFeignService.getAllRecord(condDrg);
        }
        Boolean drgFlag = (Objects.nonNull(finMaDrgDtls) && finMaDrgDtls.size()>0)?true:false;
        if(drgFlag){
            groupedDetails = finMaDrgDtls.stream().collect(Collectors.groupingBy(FinMaDrgDtl::getMainId));
        } else {
            groupedDetails = null;
        }
        Boolean drgGroupFlag = (Objects.nonNull(groupedDetails) && groupedDetails.size()>0)?true:false;
        // 使用Stream API过滤出type为DRG的枚举值
        List<FinMaMedicalItemEnum> filteredEnums = Arrays.stream(FinMaMedicalItemEnum.values())
                .filter(item -> "DRG".equals(item.getType()))
                .collect(Collectors.toList());
        result.getRows().stream().forEach(rt -> {
            ...
            if(drgGroupFlag){
                List<FinMaDrgDtl> drgDtls = groupedDetails.get(MapUtils.getLong(rt,"id"));
                if(Objects.nonNull(drgDtls) && drgDtls.size()>0){
                    filteredEnums.stream().filter(fe -> fe.getInsuranceType().intValue() == MapUtils.getInteger(rt,"insuranceType").intValue()).forEach(fe -> {
                        Optional<FinMaDrgDtl> opFmddl = drgDtls.stream().filter(dl -> dl.getBusiSign().equals(fe.getValue())).findFirst();
                        if(opFmddl.isPresent()){
                            FinMaDrgDtl fmddl = opFmddl.get();
                            rt.put("groupName" + fe.getSn(),fmddl.getGroupName());
    ...
    @RequestMapping(value = "/getStatDrgAll", method = { RequestMethod.POST })
    @ResponseBody
    public List<Map<String, Object>> getStatDrgAll(FinMaMainCondition cond){
        ...
        dictLevelLists.stream().forEach(mapDict -> {
            ...
            statEmp.put("settlementMethod", "DRG");                                                             // 医保结算方式
            ...
            condCount.setSettlementMethod("DRG");
            ...
            filteredEnums.stream().filter(item -> item.getInsuranceType() == 1).forEach(fem -> {
                ...
                List<Map<String,Object>> statData = finMaMainFeignService.getStatDrgRecord(condloop);
            ...
            statRdt.put("settlementMethod", "DRG");                                                             // 医保结算方式
    ...
    public void downLoadList(HttpServletResponse response, String paramJson) throws IOException {
        ...
        String fileName = URLEncoder.encode("DRG统计"+this.getDateYMD(), "UTF-8").replaceAll("\\+", "%20");
        response.setHeader("Content-disposition", "attachment;filename*=utf-8''" + fileName + ".xlsx");

        List<List<String>> dataList = this.getExcelDataList(cond);
        // 直接写入response的输出流中
        EasyExcel.write(response.getOutputStream())
                .head(getExcelHeadList()).sheet("DRG统计")
    ...
    private List<List<String>> getExcelHeadList() {
        ...
        String[] mergeTitles = {"老年性白内障结算标准（元）", "翼状胬肉结算标准（元）", "眼底注药结算标准（元）", "眼底玻切结算标准（元）"};
        for (String mergeTitle : mergeTitles) {
            // 对于每种费用类型，都有DRG分组、单眼、双眼三个子列
            head.add(Arrays.asList(mergeTitle, "DRG分组"));
            head.add(Arrays.asList(mergeTitle, "单眼"));
            head.add(Arrays.asList(mergeTitle, "双眼"));
        }
    ...
    private List<List<String>> getExcelDataList(FinMaMainCondition cond){
        ...
        if(Objects.nonNull(originalList) && originalList.size()>0){
            ...
            Lists.partition(originalList,splitNum).stream().forEach(partFinMaMain -> {
                ...
                FinMaDrgDtlCondition condDrg = new FinMaDrgDtlCondition();
                condDrg.setMainIdList(idLists);
                List<FinMaDrgDtl> finMaDrgDtls = finMaDrgDtlFeignService.getAllRecord(condDrg);
                partFinMaMain.stream().forEach(fmm -> {
                    ...
                    filteredEnums.stream().filter(fe1 -> fe1.getInsuranceType().intValue() == fmm.getInsuranceType().intValue()).forEach(fe2 -> {
                        Optional<FinMaDrgDtl> optFinMaDrgDtl =  finMaDrgDtls.stream().filter(fmdd ->
                                fmdd.getMainId().longValue() == fmm.getId().longValue()
                                && fmdd.getBusiSign().equals(fe2.getValue())).findFirst();
                        if(optFinMaDrgDtl.isPresent()){
                            list.add(optFinMaDrgDtl.get().getGroupName()!=null?optFinMaDrgDtl.get().getGroupName():"");
                            list.add(optFinMaDrgDtl.get().getSingleEye().toString()!=null?optFinMaDrgDtl.get().getSingleEye().toString():"");
                            list.add(optFinMaDrgDtl.get().getCoupleEye()!=null?optFinMaDrgDtl.get().getCoupleEye().toString():"");
                        }else{
                            list.add("");
                            list.add("");
                            list.add("");
    ...
    public List<List<String>> getExcelStatDataList(FinMaMainCondition cond){
        List<List<String>> resultList = new ArrayList<>();
        List<Map<String, Object>> resultStatLists = this.getStatDrgAll(cond);
