 ...
 */
@Controller
@RequestMapping("/ui/amcs/adverse/event/config/")
public class AdverseEventController extends BaseController {
    ...
    @GetMapping("/index")
    public String index() {
        return "amcs/adverseEvent/index";
    }
    ...
    public String createView(@RequestParam(value = "roleId") Long roleId,
        ...
        return "amcs/adverseEvent/create";
    }
    ...
    public Map staffBindRole(ProvinceRole provinceRole) {
        ...
        if (CollectionUtils.isNotEmpty(staffList)) {
            ...
            list.forEach(institution -> {
                String[] staffIds = provinceRole.getStaffIds().split(",");
                List<String> staffIdList = new ArrayList<>(Arrays.asList(staffIds));
                for (String staffId : staffIds) {
                    List<Map<String, Object>> roles = authorizeService.selectRolesByStaffIdAndInstId(Long.parseLong(staffId), institution.getId());
                    boolean shouldRemove = roles.stream()
                            .anyMatch(role -> role.get("roleId").equals(provinceRole.getRoleId()));
                    if (shouldRemove) {
                        staffIdList.remove(staffId);
                    }
                }
                if (!staffIdList.isEmpty()) {
                    authorizeService.create(provinceRole.getRoleId(), institution.getId(), String.join(",", staffIdList), Constants.PlatformCode);
                }
            });
